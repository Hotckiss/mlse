...
import ru.hse.autocode.utils.trie.NodeUtils;
...
public class GeneratedCodeDatasetCreator {

    public static void main(String[] args) throws Exception {
        SparseCSVBuilder.sharedInstance = new SparseCSVBuilder("generated1.csv", ExtractionConfig.nFeatures);
        GeneratedCodeDatasetCreator generatedCodeDatasetCreator = new GeneratedCodeDatasetCreator();
        ...
        try(BufferedReader br = new BufferedReader(new FileReader(file))) {
            br.lines().forEach(s -> {
                ...
                try {
                    generatedCodeDatasetCreator.run(s, url);
                } catch (Exception e) {
                    e.printStackTrace();
                }
            });
    ...
    public void run(final String repoName, final String repoURL) throws Exception {
        ...
        try (Git git = new Git(repo)) {
            ...
            RevCommit commit = allCommits.get(allCommits.size() - 1);
    ...
    private void handleFile(Repository repo, final String filePath, final String commitId) throws Exception {
        ...
        try(BufferedReader br = new BufferedReader(new InputStreamReader(in))) {
            ...
            String fileAsString = allFileBuilder.toString();
            if (fileAsString.contains("@generated") || fileAsString.contains("Generated By:JavaCC") || fileAsString.contains("by ANTLR")) {
    ...
    private void traverse(Repository repo, String filePath, CompilationUnit root, Node cur) throws GitAPIException {
    ...
    private void traverseBlock(Repository repo, String filePath, CompilationUnit root, MethodDeclaration md, Statement blk, int dep, int k) throws GitAPIException {
    ...
    private int countStmtsSeq(List<Statement> statement) {
        int res = 0;
    ...
    private int countStmts(Statement statement) {
    ...
    private boolean canExtract(Statement blk, int dep) {
    ...
    private void analyzeStatementsSeq(Repository repo, CompilationUnit root, MethodDeclaration md, List<Statement> seq, String filePath) throws GitAPIException {
        if (seq.isEmpty()) {
            return;
        }
    ...
    private void analyzeBlock(Repository repo, CompilationUnit root, MethodDeclaration md, Statement blk, String filePath) throws GitAPIException {
        SparseCSVBuilder.sharedInstance.writeVector(false);
    ...
    private void calcConnectivity(CompilationUnit root, MethodDeclaration md, List<Statement> statementsSeq) {
        String fragment = buildFragmentFromSeq(statementsSeq);
        int fragmentLocs = NodeUtils.locsString(fragment);
    ...
    private void analyzeDepth(String code, int locCount, boolean isCodeFragment) {
        int dep = 0;
        int area = 0;
        int depInLine = 0;
