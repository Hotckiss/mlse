...
class PathContextReader:
    ...
    def __init__(self, word_to_index, target_word_to_index, path_to_index, config, is_evaluating=False):
        ...
        self.reading_batch_size = config.READING_BATCH_SIZE if is_evaluating else min(config.READING_BATCH_SIZE,
                                                                                      config.NUM_EXAMPLES)
    ...
    def start(self, session, data_lines=None):
        ...
        self.threads = tf.compat.v1.train.start_queue_runners(sess=session, coord=self.coord)
        return self

    def read_file(self):
        row = self.get_row_input()
        record_defaults = [[no_such_composite]] * (self.data_num_contexts + 1)
        row_parts = tf.io.decode_csv(records=row, record_defaults=record_defaults, field_delim=' ')
        ...
        split_contexts = tf.compat.v1.string_split(flat_contexts, delimiter=',')
        dense_split_contexts = tf.reshape(tf.sparse.to_dense(split_contexts,
    ...
    def get_row_input(self):
        if self.is_evaluating:  # test, read from queue (small data)
            row = self.input_placeholder = tf.compat.v1.placeholder(tf.string)
        else:  # training, read from file
            filename_queue = tf.compat.v1.train.string_input_producer([self.file_path], num_epochs=self.num_epochs,
                                                                      shuffle=False)
    ...
    def initialize_batch_outputs(self, filtered_input):
        return tf.compat.v1.train.shuffle_batch(filtered_input,
    ...
    def get_filtered_input(self):
        ...
        any_contexts_is_valid = tf.logical_or(
            tf.greater(tf.squeeze(tf.reduce_max(input_tensor=path_source_indices, axis=1), axis=1), 0),
            tf.logical_or(
                tf.greater(tf.squeeze(tf.reduce_max(input_tensor=path_target_indices, axis=1), axis=1), 0),
                tf.greater(tf.squeeze(tf.reduce_max(input_tensor=path_indices, axis=1), axis=1), 0))
        )  # (batch, )

        if self.is_evaluating:
            cond = tf.compat.v1.where(any_contexts_is_valid)
        else:  # training
            word_is_valid = tf.greater(word_label, 0)  # (batch, )
            cond = tf.compat.v1.where(tf.logical_and(word_is_valid, any_contexts_is_valid))  # (batch, 1)
        valid_mask = tf.cast(  # (batch, max_contexts, 1)
            tf.logical_or(tf.logical_or(tf.greater(path_source_indices, 0),
                                        tf.greater(path_target_indices, 0)),
                          tf.greater(path_indices, 0)), dtype=tf.float32
        )
