...
import PathContextReader2 as PathContextReader
import numpy as np
import time
import pickle
...
class Model:
    ...
    def create_index_to_target_word_map(self):
        self.index_to_target_word_table = tf.lookup.StaticHashTable(
                tf.lookup.KeyValueTensorInitializer(list(self.index_to_target_word.keys()),
    ...
    def build_training_graph(self, input_tensors):
        ...
        with tf.compat.v1.variable_scope('model'):
            words_vocab = tf.compat.v1.get_variable('WORDS_VOCAB', shape=(self.word_vocab_size + 1, self.config.EMBEDDINGS_SIZE),
                                          dtype=tf.float32,
            ...
            target_words_vocab = tf.compat.v1.get_variable('TARGET_WORDS_VOCAB',
                                                 ...
                                                 initializer=tf.compat.v1.keras.initializers.VarianceScaling(scale=1.0,
                                                                                                            mode=('FAN_OUT').lower(),
                                                                                                            distribution=("uniform" if True else "truncated_normal")))
            attention_param = tf.compat.v1.get_variable('ATTENTION',
                                              shape=(self.config.EMBEDDINGS_SIZE * 3, 1), dtype=tf.float32)
            paths_vocab = tf.compat.v1.get_variable('PATHS_VOCAB', shape=(self.path_vocab_size + 1, self.config.EMBEDDINGS_SIZE),
                                          dtype=tf.float32,
                                          initializer=tf.compat.v1.keras.initializers.VarianceScaling(scale=1.0,
                                                                                                     mode=('FAN_OUT').lower(),
                                                                                                     distribution=("uniform" if True else "truncated_normal")))
            ...
            batch_size = tf.cast(tf.shape(input=words_input)[0], dtype=tf.float32)
            loss = tf.reduce_sum(input_tensor=tf.nn.sparse_softmax_cross_entropy_with_logits(
    ...
    def calculate_weighted_contexts(self, words_vocab, paths_vocab, attention_param, source_input, path_input,
        ...
        if not is_evaluating:
            context_embed = tf.nn.dropout(context_embed, 1 - (keep_prob1))

        flat_embed = tf.reshape(context_embed, [-1, self.config.EMBEDDINGS_SIZE * 3])  # (batch * max_contexts, dim * 3)
        transform_param = tf.compat.v1.get_variable('TRANSFORM',
        ...
        code_vectors = tf.reduce_sum(input_tensor=tf.multiply(batched_embed, attention_weights),
                                                  axis=1)  # (batch, dim * 3)
    ...
    def build_test_graph(self, input_tensors, normalize_scores=False):
        with tf.compat.v1.variable_scope('model', reuse=self.get_should_reuse_variables()):
            ...
            attention_param = tf.compat.v1.get_variable('ATTENTION',
                                              ...
                                              dtype=tf.float32, trainable=False)
            paths_vocab = tf.compat.v1.get_variable('PATHS_VOCAB',
                                          shape=(self.path_vocab_size + 1, self.config.EMBEDDINGS_SIZE),
                                          dtype=tf.float32, trainable=False)

            target_words_vocab = tf.transpose(a=target_words_vocab)  # (dim * 3, target_word_vocab+1)
        ...
        top_indices = tf.cast(topk_candidates.indices, dtype=tf.int64)
    ...
    def save_word2vec_format(self, dest, source):
        with tf.compat.v1.variable_scope('model', reuse=None):
            ...
            embeddings = tf.compat.v1.get_variable(var_name, shape=(vocab_size + 1, embedding_size), dtype=tf.float32,
                                         trainable=False)
            self.saver = tf.compat.v1.train.Saver()
            self.load_model(self.sess)
    ...
    def initialize_session_variables(sess):
        sess.run(tf.group(tf.compat.v1.global_variables_initializer(), tf.compat.v1.local_variables_initializer(), tf.compat.v1.tables_initializer()))
